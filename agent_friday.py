...
def _build_llm():
    if LLM_PROVIDER == "groq":
        logger.info("LLM → Groq (fast & free)")
        return lk_groq.LLM(
            model="llama-3.3-70b-versatile",      # excellent for voice + tools
...
def _build_tts():
    if TTS_PROVIDER == "elevenlabs":
        logger.info("TTS → ElevenLabs (free tier)")
        ...
        # fallback if you ever want to switch back
        return lk_openai.TTS(voice=OPENAI_TTS_VOICE, model=OPENAI_TTS_MODEL, speed=TTS_SPEED)
    elif TTS_PROVIDER == "sarvam":
        return sarvam.TTS(language=SARVAM_TTS_LANGUAGE, speaker=SARVAM_TTS_SPEAKER)
    else:
